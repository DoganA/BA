...
function syncData {
  ...
  ##Versuch bis zu 3 mal eine Vernbindung aufzubauen, wenn die Vebindung Fehlgeschlagen hat.
  ...
  if [ $STATUS -eq 0 ] ;then
    ...
    DATEJMT=$(date '+%Y-%m-%d')
	BYTES=$(ifconfig eth0 | grep 'RX bytes')
...
function connect {
  ...
  sudo ifup eth0 			#aktiviert den netzwerkinterface ppp0
  
  i=0
  ifplugstatus eth0 	#eth0 is the first point ot point protocol; Ethernetanschluss: eth0 
  STATUS=$?
